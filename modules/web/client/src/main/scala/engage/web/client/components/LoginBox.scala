...
import engage.web.client.Actions.LoggedIn
import engage.web.client.circuit.EngageCircuit
import engage.web.client.components.forms.FormLabel
...
object LoginBox {
  ...
  class Backend(b: BackendScope[Props, State]) {
    ...
    def loggedInEvent(u: UserDetails): Callback =
      b.setState(State.Empty) >> EngageCircuit.dispatchCB(LoggedIn(u))
    ...
    def closeBox: Callback                      =
      b.setState(State.Empty) >> EngageCircuit.dispatchCB(CloseLoginBox)

    private val attemptLogin = (e: ReactEvent, _: Form.FormProps) =>
      e.preventDefaultCB *>
        b.state >>= { s =>
