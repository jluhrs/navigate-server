...
class WebSocketHandler[M](modelRW: ModelRW[M, WebSocketConnection])
  ...
  def webSocket: Future[Action] = Future[Action] {
    ...
    val url      = s"$protocol://$host/api/engage/events"
    val ws       = new WebSocket(url)

    def onOpen(): Unit = {
      logger.info(s"Connected to $url")
      EngageCircuit.dispatch(Connected(ws, 0))
    }
    ...
    def onMessage(e: MessageEvent): Unit =
      e.data match {
        case buffer: ArrayBuffer =>
          ...
          Either.catchNonFatal(Unpickle[EngageEvent].fromBytes(byteBuffer)) match {
            case Right(event: ServerLogMessage) =>
              EngageCircuit.dispatch(AppendToLog(event))
            case Right(event)                   =>
              logger.info(s"Decoding event: ${event.getClass}")
              EngageCircuit.dispatch(ServerMessage(event))
            case Left(t)                        =>
  ...
  def connectedHandler: PartialFunction[Any, ActionResult[M]] = { case Connected(ws, delay) =>
    ...
    val handle = setInterval(5.minutes)(EngageCircuit.dispatch(VerifyLoggedStatus))
      ...
      Effect.action(
        AppendToLog(
          ServerLogMessage(ServerLogLevel.ERROR,
                           Instant.now,
                           "Error connecting to the engage server"
          )
        )
      )
    )
