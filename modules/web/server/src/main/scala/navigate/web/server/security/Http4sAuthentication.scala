...
class Http4sAuthentication[F[_]: Sync](auth: AuthenticationService[F]) extends Http4sDsl[F] {
  ...
  private val onFailure: AuthedRoutes[AuthenticationFailure, F] =
...
/**
 * Middleware used to keep the session alive as long as authenticated request are coming It has some
 * cost as it needs to decode/encode the cookie with JWT
 */
    ...
    result.fold(_ => service(request),
                u =>
                  OptionT.liftF(auth.loginCookie(u)) >>= { c =>
                    service.map(_.addCookie(c)).apply(request)
                  }
    )
  }

  def apply[F[_]: Monad](service: HttpRoutes[F], auth: Http4sAuthentication[F]): HttpRoutes[F] =
    auth.optAuthUser.flatMap(replaceCookie[F](service, auth))
}
    ...
    Sync[F]
      ...
      .map { exp =>
        ResponseCookie(name,
                       token,
                       path = "/".some,
                       expires = exp.toOption,
                       secure = ssl,
                       httpOnly = true
        )
      }

  def loginCookie[F[_]: Sync](
...
object CookiesService {
  def apply(cookieName: String, useSSL: Boolean, timeToLive: Long): CookiesService =
    new CookiesService {
