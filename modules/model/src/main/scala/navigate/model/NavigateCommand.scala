...
object NavigateCommand {
  ...
  case class TargetAdjust(target: VirtualTelescope, handsetAdjustment: HandsetAdjustment)
      extends NavigateCommand
  case class OriginAdjust(handsetAdjustment: HandsetAdjustment)   extends NavigateCommand
  ...
  extension (self: NavigateCommand) {
    def name: String = self match {
      ...
      case TargetAdjust(_, _)         => "Target Adjustment"
      case OriginAdjust(_)            => "Origin Adjustment"
